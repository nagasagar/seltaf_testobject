...
import com.seltaf.webelements.Table;
...
public abstract class BasePage {
	 ...
	 private int explictWaitTimeout = driverManager.getExplicitWait();
	 private int sessionTimeout = driverManager.getWebSessionTimeout();
	    
	 public BasePage() { }
	 
	protected abstract void assertCurrentPage(final boolean log) throws NotCurrentPageException;
	protected abstract void capturePageSnapshot();
	 ...
	 public void acceptAlert() throws NotCurrentPageException {
	    ...
	    public void assertAlertPresent() {
	    	SeltafTestLogger.logWebStep(null, "assert alert present.", false);
	        try {
	    ...
	    public void assertAlertText(final String text) {
	    	SeltafTestLogger.logWebStep(null, "assert alert text.", false);
	    ...
	    public void assertConfirmationText(final String text) {
	    	SeltafTestLogger.logWebStep(null, "assert confirmation text.", false);
	    ...
	    public void assertPromptText(final String text) {
	        SeltafTestLogger.logWebStep(null, "assert prompt text.", false);
	    ...
	    public void assertTextNotPresent(final String text) {
	        SeltafTestLogger.logWebStep(null, "assert text \"" + text + "\" is not present.", false);
	        assertHTML(!isTextPresent(text), "Text= {" + text + "} found.");
	    }

	    public void assertTextNotPresentIgnoreCase(final String text) {
	        SeltafTestLogger.logWebStep(null, "assert text \"" + text + "\" is not present.(ignore case)", false);
	        assertHTML(!getBodyText().toLowerCase().contains(text.toLowerCase()), "Text= {" + text + "} found.");
	    }

	    public void assertTextPresent(final String text) {
	        SeltafTestLogger.logWebStep(null, "assert text \"" + text + "\" is present.", false);
	        assertHTML(isTextPresent(text), "Text= {" + text + "} not found.");
	    }

	    public void assertTextPresentIgnoreCase(final String text) {
	        SeltafTestLogger.logWebStep(null, "assert text \"" + text + "\" is present.(ignore case)", false);
	 ...
	 public boolean isElementPresent(final By by) {
	        ...
	        try {
	            ...
	            if ((e instanceof InvalidSelectorException)
	            	...
	            	SeltafTestLogger.log("InvalidSelectorException or CastException got, retry");
	 ...
	 public boolean isFrame() {
	    ...
	    public void waitForElementChecked(final HtmlElement element) {
	        ...
	        SeltafTestLogger.logWebStep(null, "wait for " + element.toString() + " to be checked.", false);
	    ...
	    public void waitForPopUp(final String windowID, final String timeout) {
	        ...
	        if (driverManager.getConfig().getBrowser() == BrowserType.InternetExplore) {
	    ...
	    public void waitForTextPresent(final HtmlElement element, final String text) {
	        ...
	        SeltafTestLogger.logWebStep(null, "wait for text \"" + text + "\" to be present.", false);
	    ...
	    public void waitForTextToDisappear(final String text) {
	        Assert.assertNotNull(text, "Text can't be null");
	        SeltafTestLogger.logWebStep(null, "wait for text \"" + text + "\" to disappear.", false);
